...
class BasicQueryEnablementSpec
  ...
  def n1qlFromSql2(sql2: String): String =
    (lpLcToN1ql[Eff](compileLogicalPlan(sql2), listc) ∘ n1qlQueryString)
      .run.run.map(_._2)
  ...
  "QScript to N1QL" should {
    ...
    "convert a basic reduction" in {
      ...
      val qs =
        chain[Fix, QST](
          SRTF.inj(Const(ShiftedRead(rootDir </> file("person"), ExcludeId))),
          QCT.inj(LeftShift((),
            HoleF,
            ProjectFieldR(
              Free.point(RightSide),
    ...
    "convert a flatten array" in {
      ...
      val qs =
        chain[Fix, QST](
          SRTF.inj(Const(ShiftedRead(rootDir </> file("zips"), ExcludeId))),
          QCT.inj(LeftShift((),
            HoleF,
            ProjectFieldR(
              Free.point(RightSide),
