...
trait N1QLRenderQueryInstance {
  import N1QL._, Case._, Select._

  implicit def renderQuery[T[_[_]]: Recursive: Corecursive]: RenderQuery[N1QLT[T]] =
    new RenderQuery[N1QLT[T]] {
      implicit val codec = common.CBDataCodec

      def compact(a: N1QLT[T]): PlannerError \/ String = {
        val q = a.embed.cataM[PlannerError \/ ?, String] {
          case Data(v) =>
        ...
        a match {
          case _: Select[T[N1QL]] => q ∘ (s => s"select value v from $s v")
          case _                  => q ∘ ("select value " ⊹ _)
        }
      }

      def pretty(a: N1QLT[T]): PlannerError \/ String = compact(a)
    }
}
