...
import matryoshka._, FunctorT.ops._, Recursive.ops._, TraverseT.nonInheritedOps._
import pathy.Path._
...
object queryfile {
  ...
  def lpLcToN1ql[S[_]](
    ...
    type CBQScript0[A] = (QScriptCore[Fix, ?] :\: EquiJoin[Fix, ?] :/: Const[ShiftedRead[APath], ?])#M[A]
    type CBQScript[A] = (QScriptCore[Fix, ?] :\: EquiJoin[Fix, ?] :/: Const[ShiftedRead[AFile], ?])#M[A]

    implicit val couchbaseQScriptToQSTotal: Injectable.Aux[CBQScript, QScriptTotal[Fix, ?]] =
    ...
    for {
      ...
      qs   <- convertToQScriptRead[
                Fix,
                Plan[S, ?],
                QScriptRead[Fix, APath, ?]
              ](lc)(lp)
      _    <- tell(Vector(tree("QS post convertToQScriptRead", qs)))
      shft <- shiftRead(qs)
                .transCata(SimplifyJoin[Fix, QScriptShiftRead[Fix, APath, ?], CBQScript0].simplifyJoin(idPrism.reverseGet))
                .transCataM(ExpandDirs[Fix, CBQScript0, CBQScript].expandDirs(idPrism.reverseGet, lc))
