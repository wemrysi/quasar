...
final class ThetaJoinPlanner[F[_]: Monad: NameGenerator, T[_[_]]: Recursive: ShowT]
  extends Planner[F, ThetaJoin[T, ?]] {

  def plan: AlgebraM[M, ThetaJoin[T, ?], N1QL] = {
      ...
      for {
        ...
        _       <- prtell[M](Vector(Detail(
                     ...
                     s"""  src:     $sN1ql
                        ...
                        |  combine: $cN1ql
                        |  n1ql:    ???""".stripMargin('|'))))
      } yield partialQueryString("???ThetaJoin???")
  }
}
