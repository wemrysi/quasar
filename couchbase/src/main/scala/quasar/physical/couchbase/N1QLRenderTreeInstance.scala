...
trait N1QLRenderTreeInstance {
  ...
  implicit def renderTree[T[_[_]]: Recursive]: RenderTree[N1QLT[T]] = {
    def nonTerminal[A](typ: String, c: T[N1QL]*): RenderedTree =
      NonTerminal(typ :: Nil, none, c.toList.map(i => renderTree.render(i.project)))
    ...
    RenderTree.make {
      ...
      case Obj(m) =>
        NonTerminal("Obj" :: Nil, none, m.map { case (k, v) =>
          NonTerminal("K → V" :: Nil, none,
            List(renderTree.render(k.project), renderTree.render(v.project)))
        }.toList)
      case Arr(l) =>
        nonTerminal("Arr", l: _*)
      case Time(a1) =>
      ...
      case Select(v, re, ks, un, ft, gb, ob) =>
        def nt(tpe: String, label: Option[String], child: T[N1QL]) =
          NonTerminal(
            tpe :: Nil,
            label,
            List(renderTree.render(child.project)))
