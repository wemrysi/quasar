...
object safe {
  ...
  def anaM[M[_]: Monad, T, F[_]: Traverse, A](
      ...
      implicit T: Corecursive.Aux[T, F])
      : M[T] =
    hyloM[M, F, A, T](a)(T.embed(_).point[M], f)
  ...
  def cataM[M[_]: Monad, T, F[_]: Traverse, A](
      ...
      implicit T: Recursive.Aux[T, F])
      : M[A] =
    hyloM[M, F, T, A](t)(f, T.project(_).point[M])
  ...
  def transApoT[T, F[_]: Traverse](
    ...
    def go(v: T): Eval[T] =
      f(v).fold(Eval.now, TR.project(_).traverse(go).map(TC.embed(_)))
    ...
    final class PartiallyApplied[M[_]] {
      def apply[F[_], A, B]
        (a: A)
        (f: ElgotAlgebraM[(A, ?), M, F, B], g: CoalgebraM[M, F, A])
