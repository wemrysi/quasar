...
object CoreMap extends Serializable {
  ...
  def change[T[_[_]] : RecursiveT]: AlgebraM[PlannerError \/ ?, MapFunc[T, ?], Data => Data] = {
    ...
    case TimeOfDay(f) => (f >>> {
      case Data.Timestamp(v) => Data.Time(v.atZone(UTC).toLocalTime)
      case _ => undefined
    }).right
  ...
  private def lt(d1: Data, d2: Data): Data = (d1, d2) match {
    ...
    case (Data.Bool(false), Data.Bool(true))    => Data.Bool(true)
    case (Data.Bool(_), Data.Bool(_))           => Data.Bool(false)
    case _                                      => undefined
  }
  ...
  private def gt(d1: Data, d2: Data): Data = (d1, d2) match {
    ...
    case (Data.Date(a), Data.Date(b))           => Data.Bool(a.compareTo(b) > 0)
    case (Data.Time(a), Data.Time(b))           => Data.Bool(a.compareTo(b) > 0)
    case (Data.Bool(true), Data.Bool(false))    => Data.Bool(true)
    case (Data.Bool(_), Data.Bool(_))           => Data.Bool(false)
    case _                                      => undefined
  }
  ...
  private def gte(d1: Data, d2: Data): Data = (d1, d2) match {
    ...
    case (Data.Date(a), Data.Date(b))           => Data.Bool(a.compareTo(b) >= 0)
    case (Data.Time(a), Data.Time(b))           => Data.Bool(a.compareTo(b) >= 0)
    case (Data.Bool(false), Data.Bool(true))    => Data.Bool(false)
    case (Data.Bool(_), Data.Bool(_))           => Data.Bool(true)
    case _                                      => undefined
  }
  ...
  private def fromDateTime(v: Instant)(f: ZonedDateTime => Data): Data =
    \/.fromTryCatchNonFatal(v.atZone(UTC)).fold(κ(Data.NA), f)
