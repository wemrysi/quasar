...
class JDBMRawSortProjection private[yggdrasil](dbFile: File,
  ...
  override def getBlockAfter(id: Option[Bytes], columns: Option[Set[ColumnRef]])(
    ...
    try {
      ...
      val iterator = {
        var initial: Try[BtoBIterator] = ScalaFailure(new Exception)
        var tries                                                                 = 0
        while (tries < JDBMProjection.MAX_SPINS && initial.isFailure) {
          initial = Try(iteratorSetup())
          tries += 1
        }
        initial getOrElse { throw new VicciniException("Initial drop failed with too many concurrent mods.") }
      }
