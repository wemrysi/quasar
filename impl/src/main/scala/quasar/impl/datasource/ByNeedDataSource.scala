...
final class ByNeedDataSource[F[_], G[_], Q, R] private (
    ...
    mvar: MVar[F, NeedState[F, DataSource[F, G, Q, R]]])(
    implicit F: Async[F])
...
object ByNeedDataSource {
  ...
  def apply[F[_]: Async, G[_], Q, R](
    ...
    MVar.uncancelableOf[F, NeedState[F, DataSource[F, G, Q, R]]](
      NeedState.Uninitialized(init))
      .map(new ByNeedDataSource(kind, _))
}
