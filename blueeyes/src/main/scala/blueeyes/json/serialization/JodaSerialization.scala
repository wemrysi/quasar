...
trait JodaSerializationImplicits {
  ...
  implicit val DurationExtractor = LongExtractor map { l =>
    new JDuration(l)
  }

  implicit val DurationDecomposer = new Decomposer[JDuration] {
    def decompose(duration: JDuration): JValue = JNum(duration.getMillis)
  }
