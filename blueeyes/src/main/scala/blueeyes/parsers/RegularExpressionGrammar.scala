...
trait RegularExpressionGrammar extends RegexParsers {
  ...
  def backReference = escapeChar ~> (regex("[1-9]") +) ^^ { value =>
    BackReference(value.mkString("").toInt)
  }
  ...
  def atomicGroup = roundBracketStartChar ~> questionMarkChar ~> greaterThanChar ~> (not(roundBracketEndChar) ~> regexAtom *) <~ roundBracketEndChar ^^ {
    value =>
      AtomicGroup(value: _*)
  }
  ...
  def flags = (regex("[sim]") *) ~ ((hyphenChar ~> (regex("[sim]") *)) ?) ^^ {
    case flags1 ~ flags2 => Flags(flags1.map(_.charAt(0)), flags2.map(_.map(_.charAt(0))))
  }

  def lookAround =
    roundBracketStartChar ~> questionMarkChar ~> (lessThanChar ?) ~ (equalsChar | exclamationMarkChar) ~ (not(roundBracketEndChar) ~> regexAtom *) <~ roundBracketEndChar ^^ {
      case lookbehind ~ positive ~ value => LookAround(lookbehind.map(v => false).getOrElse(true), positive == EqualsChar, value: _*)
    }

  def group = roundBracketStartChar ~> (not(roundBracketEndChar) ~> regexAtom *) <~ roundBracketEndChar ^^ { value =>
    Group(value: _*)
  }

  def namedCaptureGroup =
    roundBracketStartChar ~> questionMarkChar ~> (lessThanChar ~> (regex("[a-zA-z0-9]") +) <~ greaterThanChar) ~ (not(roundBracketEndChar) ~> regexAtom *) <~ roundBracketEndChar ^^ {
      case name ~ value => NamedCaptureGroup(name.mkString(""), value: _*)
    }

  def nonCapturingGroup =
    roundBracketStartChar ~> questionMarkChar ~> (flags ?) ~ (colonChar ~> (not(roundBracketEndChar) ~> regexAtom *)) <~ roundBracketEndChar ^^ {
      case flags ~ value => NonCapturingGroup(flags, value: _*)
    }

  def characterClass =
    squareBracketStartChar ~> (caretChar ?) ~ ((squareBracketEndChar ~ (characterClassAtom *) ^^ { case end ~ atoms => end :: atoms }) | (characterClassAtom +)) <~ squareBracketEndChar ^^ {
      case caret ~ atoms => CharacterClass(caret.map(v => true).getOrElse(false), atoms: _*)
    }

  def characterClassAtom =
    characterClassRange |
      escapeSequence |
      shorthandCharacterClass |
      posixCharacterClass |
  ...
  def greedyQuantifier =
    questionMarkChar ^^^ GreedyQuantifier(0, Some(1)) | starChar ^^^ GreedyQuantifier(0, None) | plusChar ^^^ GreedyQuantifier(1, None) |
      (curlyBracketStartChar ~> (decimal ^^ { value =>
                GreedyQuantifier(value, Some(value))
              } | (decimal <~ commaChar) ^^ { value =>
                GreedyQuantifier(value, None)
              } | (decimal ~ commaChar ~ decimal) ^^ { case lower ~ comma ~ upper => GreedyQuantifier(lower, Some(upper)) }) <~ curlyBracketEndChar)
  ...
  def quotation =
    quotationStart ~> ((quotationStart |
              ...
              dotChar |
              digitChar |
              otherChar) *) <~ quotationEnd ^^ (quotations => Quotation(quotations: _*))

  def quotationStart = "\\Q" ^^^ QuotationStart

  def quotationEnd = "\\E" ^^^ QuotationEnd

  def posixCharacterClass: Parser[PosixCharacterClass] =
    "\\p{" ~> ("Lower" ^^^ PosixLower |
          "Upper" ^^^ PosixUpper |
          "ASCII" ^^^ PosixASCII |
          "Alpha" ^^^ PosixAlpha |
  ...
  def shorthandCharacterClass: Parser[ShorthandCharacterClass] =
    escapeChar ~> ("d" ^^^ DigitClass |
          "D" ^^^ NotDigitClass |
          "s" ^^^ SpaceClass |
          "S" ^^^ NotSpaceClass |
          "w" ^^^ WordClass |
  ...
  def boundaryMatchEscape = escapeChar ~> regex("[bBAzZG]") ^^ { char =>
    BoundaryMatchEscape(char.charAt(0))
  }
  ...
  def smallHexNumber = escapeChar ~> "x" ~> regex("[0-9a-fA-f]{2}") ^^ { v =>
    SmallHexNumber(Integer.parseInt(v, 16).toChar)
  }

  def unicodeChar = escapeChar ~> "u" ~> regex("[0-9a-fA-f]{4}") ^^ { v =>
    UnicodeChar(Integer.parseInt(v, 16).toChar)
  }
  ...
  def otherChar =
    (not(
          escapeChar | andChar | orChar | hyphenChar | caretChar | colonChar | dollarChar | squareBracketStartChar |
