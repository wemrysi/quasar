...
class IntervalHealthMonitorSpec extends Specification with blueeyes.bkka.AkkaDefaults with FutureMatchers {
  ...
  "composes into JValue" in {
    ...
    val monitorJson = JObject(
      List(
        JField("requestCount", JObject(JField(config.toString, JArray(JNum(5) :: Nil)) :: Nil)),
        JField("request", JObject(List(JField("export", JNum(2)))))))
