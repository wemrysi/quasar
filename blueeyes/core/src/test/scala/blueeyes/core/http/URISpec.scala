...
class URISpec extends Specification with URIGen with ScalaCheck {
  "URL.toString" should {
    ...
    "be created using scheme, userinfo and host" in {
      URI(Some("foo"), Some("john:smith"), Some("google"), None, None, None, None).toString mustEqual ("foo://john:smith@google")
    }
    "be created using scheme, userinfo host and port" in {
      URI(Some("foo"), Some("john:smith"), Some("google"), Some(8080), None, None, None).toString mustEqual ("foo://john:smith@google:8080")
    }
    "be created using scheme, userinfo host, port and path" in {
      URI(Some("foo"), Some("john:smith"), Some("google"), Some(8080), Some("/bar"), None, None).toString mustEqual ("foo://john:smith@google:8080/bar")
    }
    "be created using scheme, userinfo host, port, path and query" in {
      URI(Some("foo"), Some("john:smith"), Some("google"), Some(8080), Some("/bar"), Some("query=value"), None).toString mustEqual ("foo://john:smith@google:8080/bar?query=value")
    }
    "be created using scheme, userinfo host, port, path, query and fragment" in {
      URI(Some("foo"), Some("john:smith"), Some("google"), Some(8080), Some("/bar"), Some("query=value"), Some("fragment")).toString mustEqual ("foo://john:smith@google:8080/bar?query=value#fragment")
    }
  }
