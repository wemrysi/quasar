...
class URITranscodersSpec extends Specification with ScalaCheck {
  ...
  private def pathElementGen: Gen[String] =
    Gen
      .listOfN(listSize, Gen.oneOf(Gen.oneOf[Char](URITranscoders.SafePathChars.toCharArray().toList), unicodeChar, Gen.alphaChar))
      .map(v => "/" + v.mkString(""))
  ...
  private def passTest(gen: Gen[String], trascoder: SpecialCharToStringTranscoder) = forAllNoShrink(gen) { n: String =>
    trascoder.decode(trascoder.encode(n)) must be_==(n)
  }
}
