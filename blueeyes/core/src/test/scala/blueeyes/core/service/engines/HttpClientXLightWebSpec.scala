...
class HttpClientXLightWebSpec extends Specification with TestAkkaDefaults with HttpRequestMatchers with PortFinder {
  ...
  "HttpClientXLightWeb" should {
    ...
    "Support GET requests with status OK" in {
      httpClient.parameters('param1 -> "a").get[String]("") must succeedWithContent { (_: String) =>
        ok
      }
    }
    ...
    "Support POST requests with large payload with several chunks" in {
      ...
      httpClient.post[ByteChunk]("")(chunk) must succeedWithContent { (data: ByteChunk) =>
        ByteChunk.forceByteArray(data) must awaited(duration) { (data: Array[Byte]) =>
          data.length must_== expected.length * 2
        }
      }
    }
          ...
          case responses =>
            (responses.size must_== total) and
              forall(responses) { response =>
                response.status.code must_== HttpStatusCodes.OK
              }
        }
      }
    }
    ...
    "Support POST requests with body with Array[Byte]" in {
      val expected = "Hello, world"
      httpClient.post("")(StringToByteArray(expected)) must succeedWithContent { (content: Array[Byte]) =>
        ByteArrayToString(content) must_== expected
      }
    }
    ...
    "Support POST requests with body several chunks and transcoding" in {
      ...
      httpClient.post("")(content) must succeedWithContent { (data: ByteChunk) =>
        ByteChunk.forceByteArray(data) must awaited(duration) { (bytes: Array[Byte]) =>
          new String(bytes, "UTF-8") must_== "foobar"
        }
      }
    }
...
object EchoServer extends BlueEyesServer with BlueEyesServiceBuilder with HttpRequestHandlerCombinators with TestAkkaDefaults with Logging {
  ...
  private def handler(request: HttpRequest[ByteChunk]): Future[HttpResponse[String]] = {
    val params = request.parameters.toList.sorted
      .foldLeft(List[String]()) { (l: List[String], p: Tuple2[Symbol, String]) =>
        l ++ List("%s=%s".format(p._1.name, p._2))
      }
      .mkString("&")

    val headers = request.parameters
      .get('headers)
      .map { o =>
        request.headers.raw.toList.sorted
          .foldLeft(List[String]()) { (l, h) =>
