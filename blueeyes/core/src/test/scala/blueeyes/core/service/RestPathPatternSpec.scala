...
class RestPathPatternSpec extends Specification {
  ...
  "path matching and regex extraction" should {
    ...
    "match regexp with group nested in capture groups" in {
      testPath("""(/var/(?<id3>([\w_-])+)/events/(?<id4>[\w_-]+))""", List(("/var/j20b/events/impress", Map('id3 -> "j20b", 'id4 -> "impress"))), List())
    }
  }

  "path matching and symbol extraction" should {
    "match correct literal path containing a single path element" in {
      testPath("/foo", List(("/foo", Map())), List("/bar"))
    }
    "match correct literal path containing many path elements" in {
      testPath("/foo/bar/baz", List(("/foo/bar/baz", Map())), List("/bar/baz/bar"))
    }
  ...
  "shift" should {
    "shift subpath leftward by matched pattern" in {
      ...
      pattern.shift(HttpRequest(method = HttpMethods.GET, uri = "/foo/bar/baz")) mustEqual (HttpRequest(method = HttpMethods.GET, uri = "/foo/bar/baz")
            .withSubpath("/baz"))
    }
  }
