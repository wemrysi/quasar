...
class HttpRequestLoggerJsonFormatterSpec extends Specification {
  ...
  "HttpRequestLoggerJsonFormatter" should {
    ...
    "create valid json for values with client prfix" in {
      val values: List[(FieldIdentifier, Either[String, Array[Byte]])] = (MethodIdentifier(ClientToServerPrefix), Left("GET")) :: (UriIdentifier(
                                                                                                                                     ClientToServerPrefix),
                                                                                                                                   Left("/foo/bar")) :: Nil
    ...
    "create valid json for values with server prfix" in {
      val values: List[(FieldIdentifier, Either[String, Array[Byte]])] = (StatusIdentifier(ServerToClientPrefix), Left("OK")) :: (UriIdentifier(
                                                                                                                                    ServerToClientPrefix),
                                                                                                                                  Left("/foo/bar")) :: Nil
      formatter.formatLog(values) must_== (format(Nil, Nil, fields(values)))
    }
  }
  ...
  private def toStringValue(entry: List[(FieldIdentifier, Either[String, Array[Byte]])]) = entry.map {
    _ match {
