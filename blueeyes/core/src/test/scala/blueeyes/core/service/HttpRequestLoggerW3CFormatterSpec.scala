...
class HttpRequestLoggerW3CFormatterSpec extends Specification {
  ...
  "HttpRequestLoggerW3CFormatter" should {
    ...
    "format content using Base64 encoding" in {
      val encoded = formatter.formatLog(
        Tuple2[FieldIdentifier, Either[String, Array[Byte]]](ContentIdentifier(ClientToServerPrefix), Right("content".getBytes("UTF-8"))) :: Nil)
