...
class ExpirableMapSpec extends Specification {
  ...
  "ExpirableMap: containsKey is 'false' when entry is expired" in {
    val map = newMap(None, None, { hasExpired: Expirable[String, String] =>
      true
    })
  ...
  "ExpirableMap: putIfAbsent evicts when idle time is expired" in {
    val map = newMap(Some(50), None, { hasExpired: Expirable[String, String] =>
      true
    })
    map.containsValue("bar") must eventually(be_==(false))
  }
  "ExpirableMap: putIfAbsent evicts when live time is expired" in {
    val map = newMap(None, Some(500), { hasExpired: Expirable[String, String] =>
      true
    })
    map.containsValue("bar") must eventually(be_==(false))
  }
  ...
  private def newMap(timeToIdle: Option[Long] = None, timeToLive: Option[Long] = None, hasExpired: Expirable[String, String] => Boolean = {
    hasExpired: Expirable[String, String] =>
      false
  }, evict: (String, String) => Unit = { (key: String, value: String) =>
    }) = {
    val map = new ExpirableMap[String, String](
      new ConcurrentHashMap[String, Expirable[String, String]](),
      ExpirationPolicy(timeToIdle, timeToLive, MILLISECONDS),
      hasExpired,
