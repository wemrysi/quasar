...
object StageProfile extends AkkaDefaults {
  ...
  def main(args: Array[String]) {
    ...
    val messages: List[List[String]] = List.range(0, threadsCount) map { i =>
      for (j <- 0 until threadsPerMessagesType) yield (List(i.toString))
    } flatten
  ...
  private def newStage[T](timeToIdle: Option[Long] = None, timeToLive: Option[Long] = None, evict: (String, String) => Unit, capacity: Int = 10) = {
