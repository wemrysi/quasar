...
/** Convenience trait for building services with many common mixins.
  ...
  * </pre>
  */
trait BlueEyesServiceBuilderBase[T]
    extends ServiceBuilder[T]
    with HttpHeaderImplicits
    with HttpStatusImplicits
    with HttpStatusCodeImplicits
    with HttpDateImplicits
    with HttpNumberImplicits
    with HttpRequestHandlerCombinators
    with HttpRequestHandlerImplicits
    with RestPathPatternImplicits
    with ServiceDescriptorFactoryCombinators
