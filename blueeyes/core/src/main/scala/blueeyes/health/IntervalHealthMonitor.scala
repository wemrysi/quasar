...
class IntervalHealthMonitor(val intervalConfig: IntervalConfig)(implicit executor: ExecutionContext) extends HealthMonitor with FunctionsMonitor {
  private val _countsStats: ConcurrentMap[JPath, AsyncStatistic[Long, Map[Long, Double]]] =
    new ConcurrentHashMap[JPath, AsyncStatistic[Long, Map[Long, Double]]]
  ...
  private val _errorsStats: ConcurrentMap[JPath, AsyncStatistic[Long, Map[Long, Double]]] =
    new ConcurrentHashMap[JPath, AsyncStatistic[Long, Map[Long, Double]]]
  private val _sampleStats: ConcurrentMap[JPath, Sample] = new ConcurrentHashMap[JPath, Sample]
  private val _timedSampleStats: ConcurrentMap[JPath, AsyncStatistic[Long, Map[Long, Double]]] =
    new ConcurrentHashMap[JPath, AsyncStatistic[Long, Map[Long, Double]]]
  ...
  private def errorsCountJValue = {
    val errorsCountPath =
      _errorsStats.headOption.map(v => JPath(v._1.nodes.take(v._1.nodes.length - 2) ::: List(JPathField("count"), JPathField(intervalConfig.toString))))
    val errors: Future[List[List[Long]]] = Future.sequence(errorStats.values.toList.map { statistic =>
      statistic.details.map { histogram =>
        histogram.values.toList.map(_.toLong)
      }
    })
    val errorsCount = errors.map { errors =>
      ...
      JArray(
        errors
          .foldLeft(initial) { (result, e) =>
          result zip e map (v => v._1 + v._2)
        }
          .map(JNum(_)))
    }
  ...
  private def jvalueToJObject(path: JPath, value: JValue): JValue = {
    ...
    elements.tail.foldLeft(JObject(JField(nodeName(elements.head), value) :: Nil)) { (result, element) =>
      JObject(JField(nodeName(element), result) :: Nil)
    }
  }
  ...
  private def timedSampleStat(path: JPath): AsyncStatistic[Long, Map[Long, Double]] =
    createIfAbsent[JPath, AsyncStatistic[Long, Map[Long, Double]]](path, _timedSampleStats, newTimedSample(path) _)
  private def newTimedSample(path: JPath)(): AsyncStatistic[Long, Map[Long, Double]] = TimedAverageStat(intervalConfig)

  private def counterStat(path: JPath): AsyncStatistic[Long, Map[Long, Double]] =
    createIfAbsent[JPath, AsyncStatistic[Long, Map[Long, Double]]](path, _countsStats, newCounter(path) _)
  private def newCounter(path: JPath)(): AsyncStatistic[Long, Map[Long, Double]] = TimedCountStat(intervalConfig)

  private def errorStat(path: JPath): AsyncStatistic[Long, Map[Long, Double]] =
    createIfAbsent[JPath, AsyncStatistic[Long, Map[Long, Double]]](path, _errorsStats, newErrorStat(path) _)
  private def newErrorStat(path: JPath)(): AsyncStatistic[Long, Map[Long, Double]] = TimedErrorStat(intervalConfig)

  private def timerStat(path: JPath): AsyncStatistic[Long, Map[Long, Timer]] =
    createIfAbsent[JPath, AsyncStatistic[Long, Map[Long, Timer]]](path, _timersStats, newTimer(path) _)
