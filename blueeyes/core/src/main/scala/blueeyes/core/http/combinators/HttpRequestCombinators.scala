...
trait HttpRequestCombinators {
  ...
  def refineContentType[S >: T, T](f: Handler[T, S])(implicit m: Manifest[T], executor: ExecutionContext): Handler[S, S] =
    (request: HttpRequest[Future[S]]) => refineContentType(request, f)
  ...
  def requireContent[T, S](p: T => Boolean)(f: Handler[T, S])(implicit m: Manifest[T], executor: ExecutionContext): Handler[T, S] =
    (request: HttpRequest[Future[T]]) => requireContent(p, request, f)
  ...
  private def refineContentType[S >: T, T](request: HttpRequest[Future[S]], f: Handler[T, S])(implicit m: Manifest[T],
                                                                                              executor: ExecutionContext): Future[HttpResponse[S]] = {
  ...
  private def requireContent[T, S](p: T => Boolean, request: HttpRequest[Future[T]], f: Handler[T, S])(implicit m: Manifest[T],
                                                                                                       executor: ExecutionContext): Future[HttpResponse[S]] = {
    request.content match {
      case None =>
