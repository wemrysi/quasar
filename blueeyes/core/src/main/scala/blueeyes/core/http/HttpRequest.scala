...
sealed case class HttpRequest[T] private (method: HttpMethod,
                                          uri: URI,
                                          parameters: Map[Symbol, String],
                                          headers: HttpHeaders,
                                          content: Option[T],
                                          remoteHost: Option[InetAddress],
                                          version: HttpVersion,
                                          subpath: String) {
  ...
  def withUriChanges(scheme: Option[String] = this.uri.scheme,
                     userInfo: Option[String] = this.uri.userInfo,
                     host: Option[String] = this.uri.host,
                     port: Option[Int] = this.uri.port,
                     path: Option[String] = this.uri.path,
                     query: Option[String] = this.uri.query,
                     fragment: Option[String] = this.uri.fragment) = {
...
object HttpRequest {
  def apply[T](method: HttpMethod,
               uri: URI,
               parameters: Map[Symbol, String] = Map(),
               headers: HttpHeaders = HttpHeaders.Empty,
               content: Option[T] = None,
               remoteHost: Option[InetAddress] = None,
               version: HttpVersion = `HTTP/1.1`): HttpRequest[T] = {
