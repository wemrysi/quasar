...
object LanguageRanges extends RegexParsers {
  private def elementParser =
    opt(
      regex("""(?:\b|^)""".r) ~> regex("""[a-z]{2}""".r) ~ ("-" ~> regex("[a-z]+".r)) ~ ("-" ~> regex("[a-z]+".r)) <~ regex("[^,]*".r) ^^ {
      case mainType ~ subType ~ subSubType => Range(mainType, subType, subSubType)
    } |
        regex("""(?:\b|^)""".r) ~> regex("""[a-z]{2}""".r) ~ ("-" ~> regex("[a-z]+".r)) <~ regex("[^,]*".r) ^^ {
        case mainType ~ subType => Range(mainType, subType)
      } |
