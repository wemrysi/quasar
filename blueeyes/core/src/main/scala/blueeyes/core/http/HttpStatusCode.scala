...
object HttpStatusCodes {
  ...
  case object Revalidation
      extends Warning(111, "A cache returned a stale response because an attempt to revalidate the response failed, due to an inability to reach the server.")
  ...
  case object Heuristic
      extends Warning(113, "The cache heuristically chose a freshness lifetime greater than 24 hours and the response's age was greater than 24 hours.")
  ...
  case object UseProxy
      extends Redirection(
        305,
        "Many HTTP clients (such as Mozilla[7] and Internet Explorer) do not correctly handle responses with this status code, primarily for security reasons.")
  case object TemporaryRedirect
      extends Redirection(307, "In this occasion, the request should be repeated with another URI, but future requests can still use the original URI.")
  ...
  case object Conflict
      extends ClientError(409, "Indicates that the request could not be processed because of conflict in the request, such as an edit conflict.")
  ...
  case object RequestedRangeNotSatisfiable
      extends ClientError(416, "The client has asked for a portion of the file, but the server cannot supply that portion.")
  ...
  case object NotImplemented
      extends ServerError(501, "The server either does not recognize the request method, or it lacks the ability to fulfill the request.")
  ...
  case object GatewayTimeout
      extends ServerError(504, "The server was acting as a gateway or proxy and did not receive a timely request from the upstream server.")
