...
object HttpRequestLogger extends AkkaDefaults {
  ...
  /** Creates a logger from a W3 Extended Log fields directive. e.g.:
    ...
    */
  def apply[T, S](fieldsDirective: FieldsDirective)(implicit clock: Clock, req2c: T => ByteChunk, resp2c: S => ByteChunk): HttpRequestLogger[T, S] =
    new HttpRequestLogger[T, S] {
      def apply(request: HttpRequest[T], response: Future[HttpResponse[S]]): Future[List[(FieldIdentifier, Either[String, Array[Byte]])]] = {
        Future.sequence(fieldsDirective.identifiers.map(log(_, request, response)))
      }
    }

  private def log[T, S](fieldIdentifier: FieldIdentifier, request: HttpRequest[T], response: Future[HttpResponse[S]])(
      implicit clock: Clock,
      req2c: T => ByteChunk,
      resp2c: S => ByteChunk): Future[(FieldIdentifier, Either[String, Array[Byte]])] = {
    ...
    val value: Future[Either[String, Array[Byte]]] = fieldIdentifier match {
      ...
      case ContentIdentifier(prefix) =>
        prefix match {
          case ClientToServerPrefix => aggregate(request.content.map(req2c(_)))
          case ServerToClientPrefix =>
            response.flatMap[Either[String, Array[Byte]]] { response =>
              aggregate(response.content.map(resp2c(_)))
            } recover {
              case ex => Left(ex.getMessage)
            }
      ...
      case StatusIdentifier(prefix) =>
        prefix match {
          case ServerToClientPrefix =>
            response.map[Either[String, Array[Byte]]] { response =>
              Left(response.status.code.name)
            } recover {
              case ex => Left(ex.getMessage)
            }
      ...
      case HeaderIdentifier(prefix, header) =>
        def find(key: String, headers: Map[String, String]) =
          headers find { keyValue =>
            keyValue._1.toLowerCase == key.toLowerCase
          } map { keyValue =>
            keyValue._2
          } getOrElse ("")
        prefix match {
          ...
          case ServerToClientPrefix =>
            response.map[Either[String, Array[Byte]]] { response =>
              Left(find(header, response.headers.raw))
            } recover {
              case ex => Left(ex.getMessage)
            }
