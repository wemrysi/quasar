...
sealed abstract class HeaderMetadata[T <: HttpHeader](sortOrder: Int) extends Metadata(sortOrder) {
  override def compare(other: Metadata) = other match {
    case h: HeaderMetadata[_] =>
      (header, h.header) match {
    ...
    def formatMetadata(m: Metadata): Printable[String] = m match {
      ...
      case ParameterMetadata(parameter, default) =>
        Value(
          "Request Parameter",
          parameter.toString,
          Some(default.map("; a default of " + _ + " will be used if no value is specified.").getOrElse(" (required)")))
      ...
      case OrMetadata(metadata @ _ *) =>
        ...
        Append(
          Seq(Nest(ValueCaption("Parameter Type", "Parameter", "Description")), Break) ++
      ...
      case AndMetadata(metadata @ _ *) =>
        metadata match {
          case Seq(DescriptionMetadata(desc), metadata) =>
            formatMetadata(metadata) match {
