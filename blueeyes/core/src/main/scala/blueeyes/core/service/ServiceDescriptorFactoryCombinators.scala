...
trait ServiceDescriptorFactoryCombinators extends HttpRequestHandlerCombinators with RestPathPatternImplicits with Logging {
  ...
  /** Augments the service with health monitoring. By default, various metrics
    ...
    * }}}
    */
  def healthMonitor[T, S](f: HealthMonitor => ServiceDescriptorFactory[T, S])(implicit jv2t: JValue => T,
                                                                              executor: ExecutionContext): ServiceDescriptorFactory[T, S] =
    healthMonitor("/health", defaultShutdownTimeout)(f)

  def healthMonitor[T, S](pathPrefix: RestPathPattern)(
      f: HealthMonitor => ServiceDescriptorFactory[T, S])(implicit jv2t: JValue => T, executor: ExecutionContext): ServiceDescriptorFactory[T, S] =
    healthMonitor(pathPrefix, defaultShutdownTimeout)(f)
    ...
    (context: ServiceContext) =>
      {
        ...
        serviceLifecycle.copy(
          runningState = (state: S) => {
            ...
            val monitorStoppable = stoppable map { s =>
              Stoppable(monitor, s :: Nil)
            } orElse { Some(Stoppable(monitor)) }
  ...
  def help[T, S](pathPrefix: RestPathPattern)(f: => ServiceDescriptorFactory[T, S])(implicit s2t: String => T,
                                                                                    executor: ExecutionContext): ServiceDescriptorFactory[T, S] = {
            ...
            val helpService = {
              service ~
                path(pathPrefix) {
                  get {
                    encode {
                      produce(text / html) { (request: HttpRequest[T]) =>
                        {
                          Future(
                            HttpResponse[String](content = Some(ServiceDocumenter.printFormatted(context, service)(Metadata.StringFormatter, HtmlPrinter))))
                        }
                      }
                    }
                  }
  ...
  /** Augments the service with logging.
    ...
    * }}}
    */
  def logging[T, S](f: Logger => ServiceDescriptorFactory[T, S]): ServiceDescriptorFactory[T, S] = { (context: ServiceContext) =>
    {
  ...
  /** Augments the service with request/response logging.
    ...
    * }}}
    */
  def requestLogging[T, S](f: => ServiceDescriptorFactory[T, S])(implicit t2c: Bijection[T, ByteChunk],
                                                                 executor: ExecutionContext): ServiceDescriptorFactory[T, S] =
    requestLogging(defaultShutdownTimeout)(f)

  def requestLogging[T, S](shutdownTimeout: Timeout)(f: => ServiceDescriptorFactory[T, S])(implicit t2c: Bijection[T, ByteChunk],
                                                                                           executor: ExecutionContext): ServiceDescriptorFactory[T, S] = {
  ...
  /** Augments the service with a configurable root path. If this combinator
    ...
    * }}}
    */
  def configurableRoot[T, S](f: ServiceDescriptorFactory[T, S]): ServiceDescriptorFactory[T, S] = { (context: ServiceContext) =>
    {
      val underlying = f(context)
  ...
  def serviceLocator[T, S](f: ServiceLocator[T] => ServiceDescriptorFactory[T, S])(implicit httpClient: HttpClient[T]): ServiceDescriptorFactory[T, S] = {
    ...
    (context: ServiceContext) =>
      {
        f { (serviceName: String, serviceVersion: ServiceVersion) =>
          {
  ...
  private[service] class HttpRequestLoggerActor[T](fieldsDirective: FieldsDirective,
                                                   includePaths: List[Regex],
                                                   excludePaths: List[Regex],
                                                   log: RequestLogger,
                                                   formatter: HttpRequestLoggerFormatter)(implicit contentBijection: Bijection[T, ByteChunk])
      extends Actor
      with ClockSystem {
  ...
  private[service] class MonitorHttpRequestService[T](val delegate: AsyncHttpService[T, T], healthMonitor: HealthMonitor)
...
private[service] class IncludeExcludeLogic(includePaths: List[Regex], excludePaths: List[Regex]) {
  def apply(path: String) = (includePaths, excludePaths) match {
    ...
    case _ =>
      includePaths match {
        case x :: xs =>
          includePaths.foldLeft(false) { (result, regex) =>
            {
        ...
        case Nil =>
          !excludePaths.foldLeft(false) { (result, regex) =>
            result || regex.pattern.matcher(path).matches
          }
      }
  }
}
