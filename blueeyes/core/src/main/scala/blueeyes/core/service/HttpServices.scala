...
object ResponseModifier {
  ...
  implicit def responseF[F[_]: Functor, A]: ResponseModifier[F[HttpResponse[A]]] = new ResponseModifier[F[HttpResponse[A]]] {
    def modify(result: F[HttpResponse[A]])(f: HttpResponse ~> HttpResponse) = result map { r =>
      f[A](r)
    }
  }
  ...
  implicit def responseFG[F[_]: Functor, G[_]: Functor, A]: ResponseModifier[F[G[HttpResponse[A]]]] = new ResponseModifier[F[G[HttpResponse[A]]]] {
    def modify(result: F[G[HttpResponse[A]]])(f: HttpResponse ~> HttpResponse) = result map {
      _ map { r =>
        f[A](r)
      }
    }
...
class HealthMonitorService[A, B](context: ServiceContext, monitor: HealthMonitor, startTime: Long)(implicit jv2b: JValue => B)
    extends CustomHttpService[A, Future[HttpResponse[B]]] {
  val service = (req: HttpRequest[A]) =>
    Success {
      import scalaz.syntax.show._
...
class GetRangeService[T, S](val delegate: HttpService[T, RangeHeaderValues => S]) extends DelegatingService[T, S, T, RangeHeaderValues => S] {
  val service = (req: HttpRequest[T]) =>
    extractRange(req.headers.raw) flatMap { t =>
      ...
      case (name, value) if name.toLowerCase == "range" =>
        value.split("=").toList match {
          case unit :: specifiers :: Nil =>
            val ranges = specifiers.split(",").toList.map { range =>
...
class JsonpService[A, B](
    val delegate: HttpService[A, Future[HttpResponse[B]]])(implicit extractReq: String => A, extractResp: String => B, semigroup: Semigroup[B])
...
object JsonpService extends AkkaDefaults {
  ...
  private def jsonpMetadata[A](r: HttpResponse[A]) =
    JObject(
  ...
  def jsonpConvertResponse[T](response: HttpResponse[T], callback: Option[String])(implicit fromString: String => T,
                                                                                   semigroup: Semigroup[T]): HttpResponse[T] = {
    import Bijection._
...
class ProxyService[A](httpClient: HttpClient[A], filter: HttpRequest[A] => Boolean) extends CustomHttpService[A, Future[HttpResponse[A]]] {
  def service = { r: HttpRequest[A] =>
...
class CookieService[A, B](ident: Symbol, default: => Option[String], val delegate: HttpService[A, String => B])
    extends DelegatingService[A, B, A, String => B] {
  val service = (req: HttpRequest[A]) => {
    extractCookie(req, ident, default)
      .toSuccess(DispatchError(BadRequest, "No cookie was found for the identifier " + ident))
