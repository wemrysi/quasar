...
private[engines] class HttpNettyServerProvider(conf: HttpServerConfig, service: AsyncHttpService[ByteChunk, ByteChunk], executionContext: ExecutionContext)
    extends AbstractNettyServerProvider {
...
private[engines] class HttpPipelineFactory(protocol: String,
                                           host: String,
                                           port: Int,
                                           chunkSize: Int,
                                           compression: Option[CompressionLevel],
                                           service: AsyncHttpService[ByteChunk, ByteChunk],
  ...
  def getPipeline: ChannelPipeline = {
    ...
    compression foreach { c =>
      pipeline.addFirst("compression", new HttpContentCompressor(c))
    }
