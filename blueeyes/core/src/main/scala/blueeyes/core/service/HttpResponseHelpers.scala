...
trait HttpResponseHelpers {
  ...
  /** Shorthand function to create a future of an HttpResponse from the given parameters.
    ...
    * }}}
    */
  def respond[T](status: HttpStatus = HttpStatus(OK), headers: Map[String, String] = Map(), content: Option[T] = None)(
      implicit executor: ExecutionContext): Future[HttpResponse[T]] = {
  ...
  def respondLater[T](content: Future[T], headers: Map[String, String] = Map()): Future[HttpResponse[T]] = {
    content map { c =>
      HttpResponse[T](HttpStatus(OK), headers, Some(c))
    } recover {
