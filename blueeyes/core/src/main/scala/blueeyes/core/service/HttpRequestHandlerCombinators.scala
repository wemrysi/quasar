...
trait HttpRequestHandlerCombinators {
  ...
  /**
    ...
    * </pre>
    */
  def orFail[A, B](code: HttpFailure, msg: String): HttpService[A, B] = orFail { request =>
    code -> msg
  }
  ...
  def convertedParameter[A, B, E1](parameter: Symbol, convert: String => E1, default: => Option[String] = None) = { (s: HttpService[A, E1 => B]) =>
    ParameterService[A, B](parameter, default, s.map((_: E1 => B) compose convert))
  }
  ...
  def convertedCookie[A, B, E1](ident: Symbol, convert: String => E1, default: => Option[String] = None) = { (s: HttpService[A, E1 => B]) =>
    CookieService[A, B](ident, default, s.map((_: E1 => B) compose convert))
  }

  def field[B, F1 <: JValue](s1AndDefault: IdentifierWithDefault[Symbol, F1])(
      implicit mc1: Manifest[F1]): HttpService[JValue, F1 => B] => HttpService[JValue, B] = {
  ...
  /** The jvalue combinator creates a handler that accepts and produces JSON.
    ...
    */
  def jvalue[A](h: HttpService[Future[JValue], Future[HttpResponse[JValue]]])(implicit inj: A => Future[JValue],
                                                                              surj: JValue => A,
                                                                              M: Monad[Future]): HttpService[A, Future[HttpResponse[A]]] = {
