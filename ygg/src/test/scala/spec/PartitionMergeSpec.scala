...
class PartitionMergeSpec extends TableQspec {
  ...
  def testPartitionMerge = {
    ...
    val expected = jsonMany"""
      ...
      "4a"
    """
    val reducer = CReducer.Strings map (_ mkString ";")
    val tbl     = fromJson(data)
    val result  = tbl.partitionMerge('key)(_ transform (ID \ 'value \ 'a) reduce reducer map (Table constString _))

    result.value.toSeq must_=== expected
  }
}
