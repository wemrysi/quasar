...
class GrouperSpec extends TableQspec {
  ...
  private def testHistogramByValue(set: Stream[Int]) = {
    ...
    val spec = GroupingSource(tab, ID \ 'key, Some(TransSpec1.Id), groupId, GroupKeySpecSource(tic_a, 'value))
  ...
  private def testHistogramEvenOdd(set: Stream[Int]) = {
    ...
    val tab  = fromJson(data)
    val spec = GroupingSource(tab, ID \ 'key, Some(TransSpec1.Id), groupId, GroupKeySpecSource(tic_a, Map1('value, mod2)))
  ...
  private def testCtrPartialJoinAnd(rawData1: Stream[Int -> Option[Int]], rawData2: Stream[Int]) = {
    ...
    val result = Table.merge(intersection) { (key, map) =>
      for {
        ...
        fromJson(
          Stream(JObject(JField("key", JArray(key.toJValue(tic_aj) :: key.toJValue(tic_bj) :: Nil)) ::
            JField("value", JNum(gs1Json.size + gs2Json.size)) :: Nil)))
      }
    }
  ...
  private def testNonTrivial = {
    ...
    val spec = GroupingAlignment(
      ...
      DerefObjectStatic(Leaf(Source), tic_b),
      GroupingAlignment(DerefObjectStatic(Leaf(Source), tic_a), DerefObjectStatic(Leaf(Source), tic_a), fooSpec, barSpec, GroupingSpec.Intersection),
      bazSpec,
