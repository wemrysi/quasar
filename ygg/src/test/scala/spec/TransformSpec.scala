...
trait TransformSpec extends TableQspec {
  ...
  private def select(name: String): TransSpec1                                        = select(Leaf(Source), name)

  /** Generate a specs requirement that a table generated by the given sampledata,
  ...
  def testMap1ArrayObject = {
    ...
    checkTransform(
      SampleData(data.toStream),
      Map1(select("value"), lookupF1(Nil, "negate")),
      _ => expected.toStream
    )
  }
  ...
  def testDeepMap1CoerceToDouble = {
    ...
    checkTransform(
      SampleData(data.toStream),
      DeepMap1(select("value"), lookupF1(Nil, "coerceToDouble")),
      _ => expected.toStream
    )
  }
  ...
  def testMap1CoerceToDouble = {
    val data = jsonMany"""
      ...
      {"key":[4],"value":20}
    """
    val expected = Stream[Double](12, 34.5, 31.9, 20) map (n => json"$n")

    checkTransform(
      SampleData(data.toStream),
      Map1(select("value"), lookupF1(Nil, "coerceToDouble")),
      _ => expected.toStream
    )
  }
  ...
  private def F_ValueIsEven(name: String) = Map1(select(name), F_IsEven)
  ...
  def checkMap2Eq = {
    ...
    prop { (sample: SampleData) =>
      ...
      val results = toJson(table.transform {
        Map2(
          select(select("value"), "value1"),
          select(select("value"), "value2"),
          lookupF2(Nil, "eq")
        )
      })
