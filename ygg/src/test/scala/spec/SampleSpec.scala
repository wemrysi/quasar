...
class SampleSpec extends TableQspec {
  ...
  private def testSample = {
    ...
    table.sample(15, Seq(ID, ID)).copoint.toList must beLike {
      case s1 :: s2 :: Nil =>
  ...
  private def testLargeSampleSize = {
    val data = SampleData(simpleData)
    fromSample(data).sample(1000, Seq(ID)).copoint.toList must beLike {
      case s :: Nil =>
  ...
  private def test0SampleSize = {
    val data = SampleData(simpleData)
    fromSample(data).sample(0, Seq(ID)).copoint.toList must beLike {
      case s :: Nil =>
        val result = toJson(s).copoint
