  ...
  def transformBottomUp(graph: DepGraph)(f: DepGraph => DepGraph): DepGraph = {
    ...
    def transformAux(graph: DepGraph): DepGraph = {
      def inner(graph: DepGraph): DepGraph = graph match {
        ...
        case graph @ New(parent) => f(New(transformAux(parent)))

        case graph @ AbsoluteLoad(parent, jtpe) => f(AbsoluteLoad(transformAux(parent), jtpe))

        case graph @ RelativeLoad(parent, jtpe) => f(RelativeLoad(transformAux(parent), jtpe))

        case graph @ Operate(op, parent) => f(Operate(op, transformAux(parent)))

        case graph @ Reduce(red, parent) => f(Reduce(red, transformAux(parent)))

        case MegaReduce(reds, parent) => f(MegaReduce(reds, transformAux(parent)))

        case graph @ Morph1(m, parent) => f(Morph1(m, transformAux(parent)))

        case graph @ Morph2(m, left, right) => f(Morph2(m, transformAux(left), transformAux(right)))

        case graph @ Join(op, joinSort, left, right) => f(Join(op, joinSort, transformAux(left), transformAux(right)))

        case graph @ Assert(pred, child) => f(Assert(transformAux(pred), transformAux(child)))

        case graph @ Cond(pred, left, leftJoin, right, rightJoin) =>
          f(Cond(transformAux(pred), transformAux(left), leftJoin, transformAux(right), rightJoin))

        case graph @ Observe(data, samples) => f(Observe(transformAux(data), transformAux(samples)))

        case graph @ IUI(union, left, right) => f(IUI(union, transformAux(left), transformAux(right)))

        case graph @ Diff(left, right) => f(Diff(transformAux(left), transformAux(right)))
        ...
        case graph @ Distinct(parent) => f(Distinct(transformAux(parent)))

        case s @ Split(spec, child, id) => {
          val spec2  = transformSpec(spec)
          val child2 = transformAux(child)
          f(Split(spec2, child2, id))
        }

        // not using extractors due to bug
        case s: SplitGroup =>
          f(SplitGroup(s.id, s.identities, s.parentId))

        // not using extractors due to bug
        case s: SplitParam =>
          f(SplitParam(s.id, s.parentId))
      }
