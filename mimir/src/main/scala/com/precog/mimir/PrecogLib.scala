  ...
  trait PrecogLib extends ColumnarTableLib {
    ...
    object Enrichment extends Op2(PrecogNamespace, "enrichment") {
      ...
      val tpe = BinaryOperationType(
        JObjectUnfixedT,
        JUnionT(JObjectFixedT(Map("url" -> JTextT, "options" -> JObjectUnfixedT)), JObjectFixedT(Map("url" -> JTextT))),
        JObjectUnfixedT)

      def spec[A <: SourceType](ctx: MorphContext)(left: TransSpec[A], right: TransSpec[A]): TransSpec[A] = {
        trans.MapWith(trans.InnerArrayConcat(trans.WrapArray(left), trans.WrapArray(right)), new EnrichmentMapper(ctx))
      }
      ...
      class EnrichmentMapper(ctx: MorphContext) extends CMapperM[M] {
        ...
        def map(columns: Map[ColumnRef, Column], range: Range): M[Map[ColumnRef, Column]] = {
          ...
          val chunks: List[((String, Map[String, JValue]), BitSet)] = chunks0 map {
            case (row, members) =>
              val url = urls(row)
