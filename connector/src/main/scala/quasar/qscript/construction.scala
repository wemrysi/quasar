...
import qsu.{Access, QScriptUniform => QSU}
...
object construction {
  final case class Func[T[_[_]]]()(implicit birec: BirecursiveT[T]) {
    ...
    def AccessHole: FreeMapA[T, Access[Hole]] =
      Free.pure(Access.valueHole(SrcHole))
    def LeftTarget: FreeMapA[T, QSU.ShiftTarget] =
      Free.pure(QSU.LeftTarget)
    def RightTarget: FreeMapA[T, QSU.ShiftTarget] =
      Free.pure(QSU.RightTarget)
    def AccessLeftTarget(f: Hole => Access[Hole]): FreeMapA[T, QSU.ShiftTarget] =
