...
object construction {
  final case class RecFunc[T[_[_]]: BirecursiveT]() {
    ...
    def ExtractMicroseconds[A](a1: RecFreeMapA[T, A]): RecFreeMapA[T, A] =
    ...
    def ExtractMilliseconds[A](a1: RecFreeMapA[T, A]): RecFreeMapA[T, A] =
    ...
    def Date[A](a1: RecFreeMapA[T, A]): RecFreeMapA[T, A] =
      rollCore(MapFuncsCore.Date(a1))
    def Time[A](a1: RecFreeMapA[T, A]): RecFreeMapA[T, A] =
      rollCore(MapFuncsCore.Time(a1))
    def Timestamp[A](a1: RecFreeMapA[T, A]): RecFreeMapA[T, A] =
