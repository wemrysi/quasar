    ...
    "convert a simple wildcard take" in {
      ...
      qs must equal(
        QC.inj(Subset(QC.inj(Unreferenced[Fix, Fix[QS]]()).embed,
          Free.roll(QCT.inj(LeftShift(Free.roll(RTP.inj(Const[Read[APath], FreeQS](Read(rootDir </> file("bar"))))), HoleF, RightSideF))),
          Take,
      ...
      convert(lc.some, StdLib.set.Take(lpRead("/foo/bar"), lpf.constant(Data.Int(10))).embed) must
        equal(
          QC.inj(Subset(
            ...
            Free.roll(QCT.inj(LeftShift(Free.roll(RTP.inj(Const[Read[APath], FreeQS](Read(rootDir </> dir("foo") </> file("bar"))))), HoleF, RightSideF))),
            Take,
            Free.roll(QCT.inj(Map(Free.roll(QCT.inj(Unreferenced())), IntLit(10)))))).embed.some)
    }
