...
class StructuralSpecs extends Specification with ScalaCheck with ValidationMatchers {
  ...
  "ConcatOp" should {
    ...
    "type combination of arbitrary strs as str" ! prop { (st1: Type, st2: Type) =>
      ConcatOp.tpe(List(st1, st2)).map(Str contains _) must beSuccessful(true)
      ConcatOp.tpe(List(st2, st1)).map(Str contains _) must beSuccessful(true)
    }.setArbitrary1(arbStrType).setArbitrary2(arbStrType)

    "type arbitrary str || unknown as Str" ! prop { (st: Type) =>
      ConcatOp.tpe(List(st, unknown)) must beSuccessful(Str)
      ConcatOp.tpe(List(unknown, st)) must beSuccessful(Str)
    }.setArbitrary(arbStrType)

    "fold constant Strings" ! prop { (s1: String, s2: String) =>
      ConcatOp.tpe(List(Const(Data.Str(s1)), Const(Data.Str(s2)))) must
        beSuccessful(Const(Data.Str(s1 + s2)))
    }

    "type combination of arbitrary arrays as array" ! prop { (at1: Type, at2: Type) =>
      ConcatOp.tpe(List(at1, at2)).map(_.arrayLike) must beSuccessful(true)
      ConcatOp.tpe(List(at2, at1)).map(_.arrayLike) must beSuccessful(true)
    }.setArbitrary1(arbArrayType).setArbitrary2(arbArrayType)

    "type arbitrary array || unknown as array" ! prop { (at: Type) =>
      ConcatOp.tpe(List(at, unknown)).map(_.arrayLike) must beSuccessful(true)
      ConcatOp.tpe(List(unknown, at)).map(_.arrayLike) must beSuccessful(true)
    }.setArbitrary(arbArrayType)

    "fold constant arrays" ! prop { (ds1: List[Data], ds2: List[Data]) =>
      ConcatOp.tpe(List(Const(Data.Arr(ds1)), Const(Data.Arr(ds2)))) must
        beSuccessful(Const(Data.Arr(ds1 ++ ds2)))
    }

    "fail with mixed Str and array args" ! prop { (st: Type, at: Type) =>
      ConcatOp.tpe(List(st, at)) must beFailing
      ConcatOp.tpe(List(at, st)) must beFailing
    }.setArbitrary1(arbStrType).setArbitrary2(arbArrayType)

    "propagate unknown types" in {
      ConcatOp.tpe(List(unknown, unknown)) must beSuccessful(unknown)
    }
  }

  "FlattenMap" should {
    "only accept maps" ! prop { (nonMap: Type) =>
      FlattenMap.tpe(List(nonMap)) must beFailing
    }.setArbitrary(arbArrayType)

    "convert from a map type to the type of its values" in {
      FlattenMap.tpe(List(Obj(Map(), Str.some))) must beSuccessful(Str)
    }

    "untype to a map type from some value type" in {
      FlattenMap.untpe(Str) must beSuccessful(List(Obj(Map(), Str.some)))
    }
  }

  "FlattenMapKeys" should {
    "only accept maps" ! prop { (nonMap: Type) =>
      FlattenMapKeys.tpe(List(nonMap)) must beFailing
    }.setArbitrary(arbArrayType)

    "convert from a map type to the type of its keys" in {
      FlattenMapKeys.tpe(List(Obj(Map(), Int.some))) must beSuccessful(Str)
    }

    "untype to a map type from some key type" in {
      FlattenMapKeys.untpe(Str) must beSuccessful(List(Obj(Map(), Top.some)))
    }
  }

  "FlattenArray" should {
    "only accept arrays" ! prop { (nonArr: Type) =>
      FlattenArray.tpe(List(nonArr)) must beFailing
    }.setArbitrary(arbStrType)

    "convert from an array type to the type of its values" in {
      FlattenArray.tpe(List(FlexArr(0, None, Str))) must beSuccessful(Str)
    }

    "untype to an array type from some value type" in {
      FlattenArray.untpe(Str) must beSuccessful(List(FlexArr(0, None, Str)))
    }
  }

  "FlattenArrayIndices" should {
    "only accept arrays" ! prop { (nonArr: Type) =>
      FlattenArrayIndices.tpe(List(nonArr)) must beFailing
    }.setArbitrary(arbStrType)

    "convert from an array type to int" in {
      FlattenArrayIndices.tpe(List(FlexArr(0, None, Str))) must beSuccessful(Int)
    }
  ...
  "ShiftMap" should {
    "only accept maps" ! prop { (nonMap: Type) =>
      ShiftMap.tpe(List(nonMap)) must beFailing
    }.setArbitrary(arbArrayType)

    "convert from a map type to the type of its values" in {
      ShiftMap.tpe(List(Obj(Map(), Str.some))) must beSuccessful(Str)
    }

    "untype to a map type from some value type" in {
      ShiftMap.untpe(Str) must beSuccessful(List(Obj(Map(), Str.some)))
    }
  }

  "ShiftMapKeys" should {
    "only accept maps" ! prop { (nonMap: Type) =>
      ShiftMapKeys.tpe(List(nonMap)) must beFailing
    }.setArbitrary(arbArrayType)

    "convert from a map type to the type of its keys" in {
      ShiftMapKeys.tpe(List(Obj(Map(), Int.some))) must beSuccessful(Str)
    }

    "untype to a map type from some key type" in {
      ShiftMapKeys.untpe(Str) must beSuccessful(List(Obj(Map(), Top.some)))
    }
  }

  "ShiftArray" should {
    "only accept arrays" ! prop { (nonArr: Type) =>
      ShiftArray.tpe(List(nonArr)) must beFailing
    }.setArbitrary(arbStrType)

    "convert from an array type to the type of its values" in {
      ShiftArray.tpe(List(FlexArr(0, None, Str))) must beSuccessful(Str)
    }

    "untype to an array type from some value type" in {
      ShiftArray.untpe(Str) must beSuccessful(List(FlexArr(0, None, Str)))
    }
  }

  "ShiftArrayIndices" should {
    "only accept arrays" ! prop { (nonArr: Type) =>
      ShiftArrayIndices.tpe(List(nonArr)) must beFailing
    }.setArbitrary(arbStrType)

    "convert from an array type to int" in {
      ShiftArrayIndices.tpe(List(FlexArr(0, None, Str))) must beSuccessful(Int)
    }
  ...
  "UnshiftMap" should {
    "convert to a map type from some value type" in {
      UnshiftMap.tpe(List(Str)) must beSuccessful(Obj(Map(), Str.some))
    }

    "untype from a map type to the type of its values" in {
      UnshiftMap.untpe(Obj(Map(), Str.some)) must beSuccessful(List(Str))
    }
  }

  "UnshiftArray" should {
    "convert to an array type from some value type" in {
      UnshiftArray.tpe(List(Str)) must beSuccessful(FlexArr(0, None, Str))
    }

    "untype from an array type to the type of its values" in {
      UnshiftArray.untpe(FlexArr(0, None, Str)) must beSuccessful(List(Str))
    }
  }
