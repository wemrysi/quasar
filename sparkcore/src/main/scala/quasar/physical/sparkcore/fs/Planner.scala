...
object Planner {
  ...
  implicit def readFile[T[_[_]]]: Planner.Aux[T, Const[Read[AFile], ?]] =
    new Planner[Const[Read[AFile], ?]] {
      type IT[G[_]] = T[G]
      def plan(fromFile: (SparkContext, AFile) => Task[RDD[String]]) =
        (qs: Const[Read[AFile], RDD[Data]]) => {
          StateT((sc: SparkContext) => {
  ...
  implicit def readPath[T[_[_]]]: Planner.Aux[T, Const[Read[APath], ?]] =
    new Planner[Const[Read[APath], ?]] {
      type IT[G[_]] = T[G]
      def plan(fromFile: (SparkContext, AFile) => Task[RDD[String]]) = ???
    }

  implicit def shiftedread[T[_[_]], A]: Planner.Aux[T, Const[ShiftedRead[A], ?]] =
    new Planner[Const[ShiftedRead[A], ?]] {
      type IT[G[_]] = T[G]
      def plan(fromFile: (SparkContext, AFile) => Task[RDD[String]]) = ???
    }
