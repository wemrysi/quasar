...
object MongoDbQScriptPlanner {
  ...
  def plan0[T[_[_]]: Recursive: Corecursive: EqualT: ShowT: RenderTreeT,
    ...
    (for {
      qs  <- QueryFile.convertToQScriptRead[T, F, QScriptRead[T, APath, ?]](lc)(lp).liftM[GenT]
      ...
      opt <- log("QScript (Mongo-specific)")(
        shiftRead(qs)
          .transCata[MongoQScript0](SimplifyJoin[T, QScriptShiftRead[T, APath, ?], MongoQScript0].simplifyJoin(idPrism.reverseGet))
          .transCataM(ExpandDirs[T, MongoQScript0, MongoQScript].expandDirs(idPrism.reverseGet, lc)).liftM[StateT[?[_], NameGen, ?]] ∘
          (_.transAna(
