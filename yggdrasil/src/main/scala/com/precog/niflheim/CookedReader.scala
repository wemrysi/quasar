...
object CookedReader {
  def load(baseDir: File,
           ...
           segmentFormat: SegmentFormat = VersionedSegmentFormat(Map(1       -> V1SegmentFormat))): CookedReader =
    ...
    segmentsByRef.toValidationNel flatMap { (segsByRef: Map[ColumnRef, List[File]]) =>
      paths.map { path =>
        val v: ValidationNel[IOException, List[Segment]] = segsByRef
          ...
          .sequence[ValidationNel[IOException, ?], Segment]
        v map (path -> _)
      }.sequence[ValidationNel[IOException, ?], ColumnRef -> List[Segment]]
    }
  }
}
