...
abstract class KafkaShardIngestActor(shardId: String,
  ...
  protected lazy val log = LoggerFactory.getLogger("com.precog.yggdrasil.actor.KafkaShardIngestActor")
  ...
  override def preStart() = {
    log.info("Starting KafkaShardIngestActor")
    context.system.scheduler.schedule(idleDelay, idleDelay) {
      initiated += 1
      self ! GetMessages(routingActor)
    }
    log.info("Recurring ingest request scheduled")
  }
  ...
  def receive = {
    ...
    case complete @ BatchComplete(requestor, checkpoint) =>
      ...
      log.debug("BatchComplete insert. Head = %s, completed = %s".format(ingestCache.headOption.map(_._1), checkpoint))
      ...
      log.trace("Pre-firing new GetMessages on batch completion to " + requestor)
      self.tell(GetMessages(requestor), requestor)

    case BatchFailed(requestor, checkpoint) =>
      log.warn("Incomplete ingest at " + checkpoint)
      ...
      if (totalConsecutiveFailures < maxConsecutiveFailures) {
        ...
        //log.error("Halting ingest due to excessive consecutive failures at Kafka offsets: " + ingestCache.keys.map(_.offset).mkString("[", ", ", "]"))
        log.error("Skipping ingest batch due to excessive consecutive failures at Kafka offsets: " + ingestCache.keys.map(_.offset).mkString("[", ", ", "]"))
        log.error("Metadata is consistent up to the lower bound:"  + ingestCache.head._1)
        log.error("Ingest will continue, but query results may be inconsistent until the problem is resolved.")
    ...
    case GetMessages(requestor) => try {
      log.trace("Responding to GetMessages from %s starting from checkpoint %s. Running batches = %d/%d".format(requestor, lastCheckpoint, runningBatches.get, maxCacheSize))
        if (runningBatches.get < maxCacheSize) {
          ...
          readRemote(lastCheckpoint).onSuccess {
            case Success((messages, checkpoint)) =>
              if (messages.size > 0) {
                log.debug("Sending " + messages.size + " events to batch ingest handler.")
                ...
                log.trace("No new data found after checkpoint: " + checkpoint)
                runningBatches.getAndDecrement
                requestor ! IngestData(Nil)
              }

            case Failure(error)    =>
              log.error("Error(s) occurred retrieving data from Kafka: " + error.message)
              runningBatches.getAndDecrement
              requestor ! IngestErrors(List("Error(s) retrieving data from Kafka: " + error.message))
          }.onFailure {
            case t: Throwable =>
              runningBatches.getAndDecrement
              log.error("Failure during remote message read", t); requestor ! IngestData(Nil)
          }
        } else {

          log.warn("Concurrent ingest window full (%d). Cannot start new ingest batch".format(runningBatches.get))
          requestor ! IngestData(Nil)
        }
    } catch {
      case t: Throwable =>
        log.error("Exception caught during ingest poll", t)
  ...
  private def readRemote(fromCheckpoint: YggCheckpoint): Future[Validation[Error, (Vector[(Long, EventMessage)], YggCheckpoint)]] = {
    ...
    def buildBatch(
      ...
      input match {
        ...
        case (offset, ArchiveMessage(_, _, _, eventId, _)) :: tail if batch.nonEmpty =>
          log.debug("Batch stopping on receipt of ArchiveMessage at offset/id %d/%d".format(offset, eventId.uid))
          (batch, checkpoint)
        ...
        case (offset, ar @ ArchiveMessage(_, _, _, EventId(pid, sid), _)) :: tail =>
          ...
          log.debug("Singleton batch of ArchiveMessage at offset/id %d/%d".format(offset, ar.eventId.uid))
    ...
    val read = M.point {
      ...
      val rawMessages: Iterable[MessageAndOffset] = msTime({ t => log.debug("Kafka fetch from %s:%d in %d ms".format(topic, lastCheckpoint.offset, t))}) {
      ...
      val eventMessages: List[Validation[Error, (Long, EventMessage.EventMessageExtraction)]] =
        msTime({t => log.debug("Raw kafka deserialization of %d events in %d ms".format(rawMessages.size, t)) }) {
      ...
      val batched: Validation[Error, Future[(Vector[(Long, EventMessage)], YggCheckpoint)]] =
        eventMessages.sequence[({ type λ[α] = Validation[Error, α] })#λ, (Long, EventMessage.EventMessageExtraction)] map { messageSet =>
          val apiKeys: List[(APIKey, Path)] = msTime({t => log.debug("Collected api keys from %d messages in %d ms".format(messageSet.size, t)) }) {
            messageSet collect {
          ...
          val authorityCacheFutures = distinctKeys map {
            case k @ (apiKey, path) =>
              ...
              permissionsFinder.inferWriteAuthorities(apiKey, path, None) map { inferred =>
                //log.trace("Write authorities inferred on %s in %d ms".format(k, System.currentTimeMillis - inferStart))
                k -> inferred
              }
          }

          authorityCacheFutures.sequence map { cached =>
            log.debug("Computed authorities from %d apiKeys in %d ms".format(distinctKeys.size, System.currentTimeMillis - authoritiesStartTime))
            ...
            val updatedMessages: List[(Long, EventMessage)] = messageSet.flatMap {
              ...
              case (offset, -\/((apiKey, path, genMessage))) =>
                authorityCache.get((apiKey, path)) map { authorities =>
                  ...
                  log.warn("Discarding event at offset %d with apiKey %s for path %s because we could not determine the account".format(offset, apiKey, path))
                  None
                }
            }

            msTime({ t => log.debug("Batch built in %d ms".format(t)) }) {
