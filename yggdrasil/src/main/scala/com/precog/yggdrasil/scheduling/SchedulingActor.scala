...
trait SchedulingActorModule extends SecureVFSModule[Future, Slice] {
  ...
  class SchedulingActor(jobManager: JobManager[Future],
    ...
    def executeTask(task: ScheduledTask): Future[PrecogUnit] = {
      ...
      if (running.contains((task.source, task.sink))) {
        ...
        val execution = for {
          ...
          cachingResult <- platform.vfs
                            .executeAndCache(platform, basePath, task.context, QueryOptions(timeout = task.timeout), Some(task.sink), Some(task.taskName))
      ...
      case TaskComplete(id, endedAt, total, error) =>
        running.values.find(_.task.id == id) match {
          case Some(TaskInProgress(task, startAt)) =>
            error match {
              case None =>
                log.info("Scheduled task %s completed with %d records in %d millis".format(id, total, (new JodaDuration(startAt, endedAt)).getMillis))

              case Some(error) =>
                log.warn("Scheduled task %s failed after %d millis: %s".format(id, (new JodaDuration(startAt, clock.now())).getMillis, error))
            }
