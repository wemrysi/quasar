...
trait SecureVFSModule[M[+ _], Block] extends VFSModule[M, Block] {
  ...
  class SecureVFS(vfs: VFS, permissionsFinder: PermissionsFinder[M], jobManager: JobManager[M], clock: Clock)(implicit M: Monad[M])
    ...
    final def executeStoredQuery(platform: Platform[M, Block, StreamT[M, Block]],
                                 scheduler: Scheduler[M],
                                 ctx: EvaluationContext,
                                 path: Path,
                                 queryOptions: QueryOptions)(implicit M: Monad[M]): EitherT[M, EvaluationError, StoredQueryResult] = {
    ...
    def executeAndCache(platform: Platform[M, Block, StreamT[M, Block]],
                        ...
                        cacheAt: Option[Path],
                        jobName: Option[String] = None)(implicit M: Monad[M]): EitherT[M, EvaluationError, StoredQueryResult] = {
      import EvaluationError._
                       ...
                       _ <- EitherT {
                             permissionsFinder.writePermissions(ctx.apiKey, cachePath, clock.instant()) map { pset =>
                               ...
                               if (pset.nonEmpty) \/.right(PrecogUnit)
                                 ...
                                 \/.left(
                                   storageError(
                                     PermissionsError("API key %s has no permission to write to the caching path %s.".format(ctx.apiKey, cachePath)))
                                 )
                             }
                           }
                       job <- EitherT.right(
    ...
    def persistingStream(apiKey: APIKey, path: Path, writeAs: Authorities, jobId: Option[JobId], stream: StreamT[M, Block], clock: Clock)(
