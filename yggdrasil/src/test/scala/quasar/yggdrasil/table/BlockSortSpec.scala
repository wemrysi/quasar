...
trait BlockSortSpec extends SpecificationLike with ScalaCheck {
  def testSortDense(sample: SampleData, sortOrder: DesiredSortOrder, unique: Boolean, sortKeys: JPath*) = {
    ...
    val original = if (unique) {
      sample.data.map(JValue.fromRValue(_)).map { jv => JArray(sortKeys.map(_.extract(jv \ "value")).toList) -> jv }.toMap.toList.unzip._2.toStream
    } else {
      sample.data.map(JValue.fromRValue(_))
    }
    ...
    result.toList.map(JValue.fromRValue(_)) must_== sorted
  ...
  def homogeneousSortSample = {
    val sampleData = SampleData(
      ...
      ]""") --> classOf[JArray]).elements.toStream.flatMap(JValue.toRValue),
      Some(
  ...
  def emptySort = {
    val sampleData = SampleData(
      (JParser.parseUnsafe("""[]""") --> classOf[JArray]).elements.toStream.flatMap(JValue.toRValue),
      Some(
        (1 , List())
      )
    )
